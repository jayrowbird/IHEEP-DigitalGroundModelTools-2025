...
namespace TriangleNet
{
    ...
    public class Mesh
    {
        ...
        /// </summary>
        public InputGeometry Load(string filename)
        {
        ...
        //Add by SB 2025 to get convex hull points
        public List<Vertex> GetConvexHull()
        {
            List<Vertex> hull = new List<Vertex>();
            Osub subseg = default(Osub);

            //foreach (var item in subsegs.Values)
            for (int i = 0; i < subsegs.Values.Count; i++)
            {
                subseg.seg = subsegs.Values.ElementAt(i);
                hull.Add(subseg.Org());
                //if (i < 1)
                //{
                //    hull.Add(subseg.Dest());
                //}
            }
            hull.Add(hull[0]);
            return hull;
        ...
        public LocateResult FindContainingTriangle(Point newvertex, ref Triangle tri)
        {
            Otri horiz = default(Otri);
            // Find a boundary triangle.
            horiz.triangle = dummytri;
            horiz.orient = 0;
            horiz.SymSelf();
            LocateResult intersect = locator.Locate(newvertex, ref horiz);
            tri = horiz.triangle;
            return intersect;
        }

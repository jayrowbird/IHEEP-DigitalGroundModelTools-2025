...
namespace TriangleNet.IO
{
    ...
    public static class FileWriter
    {
        ...
        /// <summary>
        /// Write the triangulation to a WaveFront .OBJ file.
        /// </summary>
        /// <param name="mesh"></param>
        /// <param name="filename"></param>
        /// <remarks>
        ...
        /// Center's Geomview package.
        /// </remarks>
        public static void WriteWaveFrontOBJFile(Mesh mesh, string filename)
        {
            Otri tri;
            Vertex p1, p2, p3;
            ...
            using (StreamWriter writer = new StreamWriter(filename))
            {
                Console.WriteLine("OBJ files");
                Console.WriteLine("{0}  {1}  {2}", outvertices, mesh.triangles.Count, mesh.edges);
                ...
                foreach (var item in mesh.vertices.Values)
                {
                    ...
                    {
                        // The "0.0" is here because the OFF format uses 3D coordinates.
                        writer.WriteLine("v {0}  {1}  {2}", p1[0].ToString(nfi), p1[1].ToString(nfi), p1[2].ToString(nfi));
                ...
                foreach (var item in mesh.triangles.Values)
                {
                    ...
                    // The "3" means a three-vertex polygon.
                    writer.WriteLine("f   {0}  {1}  {2}", p1.ID.ToString(nfi), p2.ID.ToString(nfi), p3.ID.ToString(nfi));
                }
            }
        }
    }

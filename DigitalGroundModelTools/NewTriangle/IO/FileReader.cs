...
namespace TriangleNet.IO
{
    ...
    public static class FileReader
    {
        ...
        static void ReadVertex(InputGeometry data, int index, string[] line, int attributes, int marks, bool is3D)
        {
            if (is3D)
            {
                //Read 3D vertice
                ReadVertex3D(data, index, line, attributes, marks);
            }
            else
            {
        ...
        static void ReadVertex3D(InputGeometry data, int index, string[] line, int attributes, int marks)
        {
            //int parseCount = 3;
            double x = double.Parse(line[1], nfi);
            double y = double.Parse(line[2], nfi);
            double z = double.Parse(line[3], nfi);
            ...
            for (int j = 0; j < attributes; j++)
            {
                if (line.Length > 4 + j)
                {
                    attribs[j] = double.Parse(line[4 + j]);
                }
            }

            // Read a vertex marker.
            if (marks > 0 && line.Length > 4 + attributes)
            {
                mark = int.Parse(line[4 + attributes]);
            }
        ...
        public static InputGeometry ReadNodeFile(string nodefilename, bool readElements)
        {
            ...
            using (StreamReader reader = new StreamReader(nodefilename))
            {
                ...
                if (line.Length > 1)
                {
                    ...
                    {
                        is3D = true;
                        Console.WriteLine("Triangle modified to work with 2.5-dimensional meshes.");
                ...
                if (invertices > 0)
                {
                    for (int i = 0; i < invertices; i++)
                    {
                        ...
                        ReadVertex(data, i, line, attributes, nodemarkers, is3D);
                    }
                }
            }
        ...
        //-----
        /// <summary>
        /// Read the vertices's from a file, which may be a .obj file.
        /// </summary>
        /// <param name="objfilename"></param>
        /// <param name="readElements"></param>
        public static void ReadOBJFile(string objfilename, out InputGeometry data, out List<ITriangle> triangles)
        {
            data = new InputGeometry();
            triangles = new List<ITriangle>();

            //this sucks but i need the vert count and face count
            string text = File.ReadAllText(objfilename);
            int linecount = text.Count(x => x == '\n');

            string[] line;
            int invertices = text.Count(x => x == 'v'); 
            int infaces = text.Count(x => x == 'f'); ;

            using (StreamReader reader = new StreamReader(objfilename))
            {
                //if (!TryReadLine(reader, out line))
                //{
                //    throw new Exception("Can't read input file.");
                ...
                //throw new Exception("Finish This Steve.");
                ...
                if (invertices > 0)
                {
                    ...
                    for (int i = 0; i < linecount; i++)
                    {
                        ...
                        if (line.Length < 3)
                        {
                            if (line[0].StartsWith("p")) continue;
                            else
                            {
                                throw new Exception("Invalid vertex.");
                            }
                        ...
                        ///Note this parse only the simplest obj files
                        ///f can and usually does include indexes to uv and normal corrdinates
                        ///sb to finish this work
                        if (line[0].ToLower().Trim() == "f")
                        {
